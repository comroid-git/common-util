...
public class VarBindTest {
    ...
    @Before
    public void setup() {
        final BufferedReader reader = new BufferedReader(new InputStreamReader(ClassLoader.getSystemResourceAsStream("dummy/message.json")));

        JSONObject data = JSON.parseObject(reader.lines().collect(Collectors.joining()));
    ...
    public void testGetters() {
        ...
        assertEquals(0, message.getVar(Message.Binds.MENTIONED_USERS).size());
