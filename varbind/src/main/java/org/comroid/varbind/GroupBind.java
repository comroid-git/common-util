...
public final class GroupBind<BAS, OBJ extends BAS, ARR extends BAS> {
    ...
    public <R extends VariableCarrier<?, ?, D>, D> BiFunction<D, OBJ, R> autoRemapper(Class<R> resultType, Class<D> dependencyType) {
    ...
    private <T> Function<OBJ, T> typeExtractor(final Class<T> target) {
        return node -> seriLib.dummy(node).getValueAs(null, target);
    }
    ...
    public final <T, X> VarBind.Duo<OBJ, T, X> bind2Stage(
        ...
        final VarBind.Duo<OBJ, T, X> bind = new VarBind.Duo<>(seriLib, this, name, extractor, remapper);
    ...
    public final <T, X, Y> VarBind.Dep<OBJ, T, X, Y> bindDependent(
        ...
        final VarBind.Dep<OBJ, T, X, Y> bind = new VarBind.Dep<>(seriLib, this, name, extractor, resolver);
    ...
    public final <T, C extends Collection<T>> ArrayBind.Uno<OBJ, T, C> list1Stage(
        ...
        final ArrayBind.Uno<OBJ, T, C> bind = new ArrayBind.Uno<>(
                seriLib,
                this,
                name,
    ...
    public final <T, R, C extends Collection<R>> ArrayBind.Duo<OBJ, T, R, C> list2Stage(
        ...
        final ArrayBind.Duo<OBJ, T, R, C> bind = new ArrayBind.Duo<>(
                seriLib,
                this,
                name,
                seriLib.arrayExtractor,
                dataExtractor,
                (aVoid, it) -> remapper.apply(it),
                collectionProvider
        );
    ...
    public final <T, R, Y, C extends Collection<R>> ArrayBind.Dep<OBJ, T, R, Y, C> listDependent(
        ...
        final ArrayBind.Dep<OBJ, T, R, Y, C> bind = new ArrayBind.Dep<>(
                seriLib,
                this,
                name,
                seriLib.arrayExtractor,
