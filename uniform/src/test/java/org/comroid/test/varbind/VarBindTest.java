...
public class VarBindTest {
    ...
    public void testGetters() {
        assertEquals(1, message.get(Message.Binds.REACTIONS).size());
        assertFalse(message.get(Message.Binds.TTS));
        assertFalse(message.get(Message.Binds.TIMESTAMP).isEmpty());
        ...
        assertEquals(0, message.get(Message.Binds.EMBEDS).size());
        assertEquals(0, message.get(Message.Binds.ATTACHMENTS).size());
        assertFalse(message.getEditedTimestamp().isPresent());
        assertNotNull(message.get(Message.Binds.AUTHOR));
        assertEquals(0, message.get(Message.Binds.MENTIONED_ROLES).size());
        ...
        assertEquals(0, message.get(Message.Binds.MENTIONED_USERS).size());
