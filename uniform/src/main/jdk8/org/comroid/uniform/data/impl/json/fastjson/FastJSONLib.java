...
public final class FastJSONLib extends SeriLib<JSON, JSONObject, JSONArray> {
    ...
    private FastJSONLib() {
        super(
                parser, JSONObject.class, JSONArray.class,
                (jsonObject, key) -> jsonObject.getJSONArray(key), JSONArray::size, node -> {
    ...
    @Override
    protected <TAR extends JSON> NodeDummy<SeriLib<JSON, JSONObject, JSONArray>, JSON, JSONObject, JSONArray, TAR> createNodeDummy(
            TAR node
    ) {
        return new NodeDummy<SeriLib<JSON, JSONObject, JSONArray>, JSON, JSONObject, JSONArray, TAR>(
                this, node) {
            @Override
            ...
            public <T> T getValueAs(final String fieldName, final Class<T> targetType) {
                return process(obj -> fieldName == null && targetType.equals(JSONObject.class)
                                       ? (T) obj
