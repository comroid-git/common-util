...
public abstract class NodeDummy<SERI extends SeriLib<BAS, OBJ, ARR>, BAS, OBJ extends BAS, ARR extends BAS, TAR extends BAS> {
    ...
    protected final <R> R process(Function<OBJ, R> objMapper, Function<ARR, R> arrMapper) {
        if (type.typ == DataStructureType.Primitive.OBJECT) return objMapper.apply(obj());
        if (type.typ == DataStructureType.Primitive.ARRAY) return arrMapper.apply(arr());
    ...
    private <T extends BAS> T tryReturn(DataStructureType<SeriLib<BAS, OBJ, ARR>, BAS, T> type)
            throws StructureTypeMismatchException {
        if (!type.equals(this.type)) throw new StructureTypeMismatchException(type, this.type);
